...
void sensor_core_task(void)
{
    ...
    while (true)
    {
        ...
        if (adc_reader_read_raw(&sensor_adc_left, &adc_raw_left) == 0 &&
            adc_reader_read_raw(&sensor_adc_right, &adc_raw_right) == 0)
        {
            if (ret == 0)
            {
